...
void AdvancedObstacleAnalyzer::computeLayeredWarning(
    ...
    int corridor_points = 0;
    int end_obstacle_points = 0;
    int outside_corridor_points = 0;  // 新增：统计框架外点数
    ...
    for (const auto& pt : cleaned_cloud->points) {
        ...
        if (in_corridor) {
            ...
            // 框架外的点云：设置为蓝色或灰色
            outside_corridor_points++;
            colored_pt.r = 128; colored_pt.g = 128; colored_pt.b = 255;  // 浅蓝色
            // 或者使用灰色：colored_pt.r = 128; colored_pt.g = 128; colored_pt.b = 128;
        }
    ...
    ROS_INFO("[AdvancedObstacleAnalyzer] Warning analysis: Corridor=%d, Outside=%d, EndObstacles=%d, Total=%zu", 
             corridor_points, outside_corridor_points, end_obstacle_points, cleaned_cloud->size());
}
